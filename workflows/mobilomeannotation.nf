...
workflow MOBILOMEANNOTATION {
    ...
    */
    ...
    def db_icefinder_hmm_models = channel.fromPath("${params.icefinder_hmm_models}.*", checkIfExists: true)
        .collect()
        .map { ice_db_files ->
            [[id: file(params.icefinder_hmm_models).name], ice_db_files]
        }

    def db_icefinder_macsyfinder_models = file(params.icefinder_macsyfinder_models, checkIfExists: true)

    def db_icefinder_prokka_uniprot = channel.fromPath("${params.icefinder_prokka_uniprot_db}/*", checkIfExists: true)
        .collect()
    ...
    ***********************************************************************************************/
    ...
    // Samples without user-provided IPS will use local-cd-search (CDD) inside PATHOFACT2.
    // .first() converts each single-file queue to a value channel so it broadcasts
    // to every sample task instead of being consumed by the first sample only.
    ...
    def ch_deeparg_db       = params.skip_deeparg
        ? Channel.empty()
        : channel.fromPath(file(params.deeparg_db, checkIfExists: true)).first()
    def ch_rgi_db           = params.skip_rgi
        ? Channel.empty()
