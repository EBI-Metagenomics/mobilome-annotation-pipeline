...
include { REFORMAT_AMRFINDER_TSV           } from '../../../modules/local/reformat_amrfinder_tsv'
...
workflow AMR_ANNOTATION {
    ...
    skip_rgi                  // boolean
    ch_user_amrfinder_tsv    // channel: tuple( val(meta), path(tsv) ) — catalogue TSV; empty if no manifest
    ...
    // AMRFinderPlus: three-way selection
    //   1. manifest_provided  → reformat catalogue TSV (column layout differs); skip AMRFINDERPLUS_RUN
    //   2. !manifest_provided && !skip_amrfinderplus → run AMRFINDERPLUS_RUN internally
    //   3. !manifest_provided && skip_amrfinderplus  → no results
    ch_amrfinderplus_results = channel.empty()

    if (manifest_provided) {
        REFORMAT_AMRFINDER_TSV(ch_user_amrfinder_tsv)
        ch_amrfinderplus_results = REFORMAT_AMRFINDER_TSV.out.tsv
    } else {
        if (!skip_amrfinderplus && ch_amrfinderplus_db) {
            // db provided externally; no-op
    ...
    def ch_gff_keyed = ch_gff.map { meta, gff -> [meta.id, meta, gff] }
    ...
    ch_amrfinder_flags = (skip_amrfinderplus && !manifest_provided) ?
        channel.empty() :
    ...
    // Active when the tool ran internally OR when catalogue results were provided via manifest.
    if (manifest_provided || !skip_amrfinderplus) {
        ...
        ch_for_amrintegrator = ch_for_amrintegrator
            ...
            .map { id, meta, gff, deeparg, rgi, amrfinder ->
                [id, meta, gff, deeparg, rgi, amrfinder ?: []]
            }
    } else {
        ch_for_amrintegrator = ch_for_amrintegrator
            .map { id, meta, gff, deeparg, rgi ->
                [id, meta, gff, deeparg, rgi, []]
            }
    }
