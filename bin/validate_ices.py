...
def parse_macsyfinder_results(macsyfinder_file, completeness_threshold=0.7, verbose=False):
    """
    ...
    try:
                ...
                if len(fields) >= 9:  # Make sure we have enough fields
                    ...
                    try:
                        ...
                        if verbose and fields[8] != 'sys_wholeness':  # Skip header
                            print(f"Warning: Invalid wholeness value '{fields[8]}' on line {line_num}", file=sys.stderr)
                        continue
                    
                    all_systems.add(sys_id)
                    
                    if verbose:
                        print(f"Line {line_num}: sys_id={sys_id}, sys_wholeness={sys_wholeness}", file=sys.stderr)
        ...
        print(f"Warning: MacSyFinder file '{macsyfinder_file}' not found. Skipping gene-based filtering.", file=sys.stderr)
        return {}, {}, set(), set()
    except Exception as e:
        print(f"Warning: Error parsing MacSyFinder file: {e}. Skipping gene-based filtering.", file=sys.stderr)
        return {}, {}, set(), set()
    
    if verbose:
        print(f"Found {len(all_systems)} total systems, {len(complete_systems)} systems >= {completeness_threshold} completeness", file=sys.stderr)
...
def map_ice_to_macsyfinder_system(ice_system_id, macsyfinder_systems, verbose=False):
    ...
    if len(ice_parts) >= 4:
        ...
        for mac_sys_id in macsyfinder_systems:
            if ice_type in mac_sys_id and ice_number in mac_sys_id:
                if verbose:
                    print(f"Mapped {ice_system_id} -> {mac_sys_id} (type: {ice_type}, number: {ice_number})", file=sys.stderr)
                return mac_sys_id
    ...
    for mac_sys_id in macsyfinder_systems:
        ...
        if ice_system_id in mac_sys_id:
            if verbose:
                print(f"Mapped {ice_system_id} -> {mac_sys_id} (ICE in MacSyFinder)", file=sys.stderr)
            return mac_sys_id
        
        # Check if MacSyFinder system ID is contained in ICE system ID
        if mac_sys_id in ice_system_id:
            if verbose:
                print(f"Mapped {ice_system_id} -> {mac_sys_id} (MacSyFinder in ICE)", file=sys.stderr)
            return mac_sys_id
    
    if verbose:
        print(f"No mapping found for {ice_system_id}", file=sys.stderr)
        print(f"Available MacSyFinder systems: {list(macsyfinder_systems)}", file=sys.stderr)
    return None

def filter_overlapping_ices_icefinder2(valid_ices, macsyfinder_file, completeness_threshold=0.7, verbose=False):
    """
    Filter overlapping ICEs based on actual icefinder2 subset logic using MacSyFinder results
    """
    # Parse MacSyFinder results
    system_genes, system_gene_names, all_systems, complete_systems = parse_macsyfinder_results(
        macsyfinder_file, completeness_threshold, verbose)
    
    if not system_genes:
        if verbose:
            print("No MacSyFinder data found, skipping gene-based filtering", file=sys.stderr)
        return valid_ices, []
    ...
    if verbose:
        print(f"Mapping {len(valid_ices)} ICE predictions to MacSyFinder systems...", file=sys.stderr)
    ...
    for ice in valid_ices:
        ...
        if mac_sys_id:
            ...
            if verbose:
                print(f"Mapped {system_id} ({ice_type}) -> {mac_sys_id} with {len(system_genes[mac_sys_id])} genes", file=sys.stderr)
        else:
            unmapped_systems.append(system_id)
            if verbose:
                print(f"Could not map {system_id} to any MacSyFinder system", file=sys.stderr)
    ...
    for ime_id, ime_genes in ime_systems.items():
        if ime_id in ice_to_system:
            ...
            if verbose:
                print(f"Checking IME {ime_id} gene requirements: {gene_names}", file=sys.stderr)
            ...
            if check_ime_gene_requirements(gene_names):
                ...
                for ice in valid_ices:
                    if ice['system_id'] == ime_id:
                        ...
                        if verbose:
                            print(f"IME {ime_id} rejected: lacks required gene composition", file=sys.stderr)
                        break
    ...
    if verbose:
        print(f"Applying subset filtering to {len(valid_ime_systems)} valid IMEs against {len(ice_systems)} ICEs", file=sys.stderr)
    ...
    for ime_id in valid_ime_systems:
        ...
        for ice_id, ice_genes in ice_systems.items():
            ...
            if ime_genes.issubset(ice_gene_set):
                ...
                if verbose:
                    print(f"  IME {ime_id} is NOT subset of ICE {ice_id}", file=sys.stderr)
    ...
    filtered_ices = []
    rejected_system_ids = {rejected['row_data']['system_id'] for rejected in rejected_overlaps}
    ...
    if verbose:
        print(f"Final result: {len(filtered_ices)} ICEs retained, {len(rejected_overlaps)} rejected", file=sys.stderr)
...
def main():
    ...
    parser.add_argument('-m', '--macsyfinder', help='MacSyFinder all_systems.tsv file for gene-based filtering')
    ...
    parser.add_argument('--no-gene-filter', action='store_true', help='Skip gene-based subset filtering')
    ...
    try:
        with open(args.input_file, 'r', newline='') as infile:
            ...
            has_header = any(header in first_line_fields for header in expected_headers[:3])
    ...
    if args.macsyfinder and not args.no_gene_filter and len(valid_ices) > 1:
        if args.verbose:
            print(f"Applying icefinder2 gene-based subset filtering to {len(valid_ices)} valid ICEs...", file=sys.stderr)
        ...
        if args.verbose:
            print(f"After gene-based filtering: {len(valid_ices)} ICEs retained, {len(rejected_overlaps)} rejected due to gene subset rules", file=sys.stderr)
    elif not args.macsyfinder and args.verbose:
        print("Warning: No MacSyFinder file provided. Skipping gene-based subset filtering.", file=sys.stderr)
        ...
        if rejected_ices:
            ...
            for rejected in rejected_ices:
                for reason in rejected['rejection_reasons'].split('; '):
                    ...
                    if 'subset' in reason.lower() or 'gene composition' in reason.lower():
