...
def outliers_parser(comp_bed, mge_data, rnas_coord):
    RNA_COV_THRESHOLD = 1.0
    CO_COV_THRESHOLD = 0.75
    mge_counter = 0
    co_repeats = {}
    ...
    with open(comp_bed, "r") as input_table:
        lines = (line for line in input_table if not line.startswith("#"))
        for line1, line2 in zip(lines, lines):
            
            # Collecting repeats info
            contig, repeat1_start, repeat1_end, co_info1 = line1.rstrip().split("\t")
            ...
            contig, repeat2_start, repeat2_end, co_info2 = line2.rstrip().split("\t")
            ...
            co_attributes = ';'.join(["mobile_element_type=compositional_outlier", gc.replace(':','='), kmer.replace(':','='), conf.replace(':','=')]) 
            new_co_id = co_contig + '|compositional_outlier:' + str(co_start) + ':' + str(co_end)
            ...
            # If no RNAs in CO then evaluate overlapping with integrated MGEs
                    ...
                    for mge_coord_pair in integrated_dic[co_contig]:
                        ...
                        if intersection > 0:
                            ...
                            if co_cov >= CO_COV_THRESHOLD:
                                mge_flag = 1
                                print(co_id + " removed due to overlapping with MGE with coverage " + str(co_cov))
