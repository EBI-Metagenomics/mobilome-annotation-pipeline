...
def mob_parser(mobilome):
    ...
    for protein_id in prots_loc:
        ...
        if len(mges_loc)>0:
            ...
            print(f"No MGE in the protein coordinates for protein {protein_id} in contig {prot_contig}")
...
def location_parser(amr_data, mob_prots, mob_coords, mob_types, user_genes):
    ...
    with open("amr_location.tsv", "w", newline='') as to_output:
        writer = csv.writer(to_output, delimiter='\t', quoting=csv.QUOTE_NONE, escapechar='\\', newline='')
        
        # Header
        headers_with_user_gene = [
            "user_gene_id", "prokka_gene_id", "contig_id", "gene_start", "gene_end",
            "ref_coverage", "ref_identity", "class_summary", "ref_gene_name",
            "ref_sequence_name", "location"
        ]
        headers_without_user_gene = [
            "gene_id", "contig_id", "gene_start", "gene_end", "ref_coverage", 
            "ref_identity", "class_summary", "ref_gene_name", "ref_sequence_name", 
            "location"
        ]
