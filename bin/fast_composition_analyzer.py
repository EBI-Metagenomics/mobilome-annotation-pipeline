...
class FastKmerAnalyzer:
    ...
    def _product(self, bases, k):
        ...
        return list(it.product(*[bases for _ in range(k)]))
            ...
            if total_kmers > 0:
                for kmer in self.all_kmers[k]:
                    ...
                    combined_profile[key] = (
                        kmer_counts.get("".join(kmer), 0) / total_kmers
                    )

        return combined_profile

    def profile_to_vector(self, profile: Dict[str, float]) -> np.ndarray:
        # convert profile to sorted numpy vector for distance calculations
        return np.array([v for _, v in sorted(profile.items(), key=lambda x: x[0])])
...
class FastCompositionAnalyzer:
    ...
    def sliding_window_analysis(self, contig: ContigInfo) -> List[Window]:
        ...
        for start in range(0, seq_len - self.window_size + 1, self.step_size):
            ...
            windows.append(Window(start, end, gc_content, kmer_profile, 0.0, 0.0, 0.0))
    ...
    def calculate_fast_outlier_scores(self, windows: List[Window]) -> List[Window]:
        ...
        kmer_vectors = np.array(
            [self.kmer_analyzer.profile_to_vector(w.kmer_profile) for w in windows]
        )
        ...
        for i, window in enumerate(windows):
            gc_z_score = (
                abs(window.gc_content - mean_gc) / std_gc if std_gc > 0 else 0.0
            )
            kmer_z_score = (
                abs(kmer_distances[i] - mean_kmer_dist) / std_kmer_dist
                if std_kmer_dist > 0
                else 0.0
            )
    ...
    def merge_overlapping_outliers(self, outliers: List[Outlier]) -> List[Outlier]:
        ...
        for current in sorted_outliers[1:]:
            ...
            if current.start <= last_merged.end + 2000:
                ...
                new_combined_score = max(
                    last_merged.combined_score, current.combined_score
                )
        ...
        return [
            outlier
            for outlier in merged
            if (outlier.end - outlier.start) >= self.min_region_size
        ]
...
class FastRepeatFinder:
    ...
    def find_exact_matches(self, seq1, seq2, is_inverted=False):
        ...
        for length in range(
            self.min_repeat_length,
            min(len(seq1), len(seq2), self.max_repeat_length) + 1,
        ):
    ...
    def _filter_best_matches(self, matches):
        ...
        for pos1, pos2, length, sequence in matches:
            ...
            if not (range1 & used_positions_seq1) and not (
                range2 & used_positions_seq2
            ):
    ...
    def find_flanking_repeats(
        self, sequence: str, outlier: Outlier
    ) -> Optional[Tuple[Repeat, Repeat]]:
        left_flank, right_flank = self.extract_flanking_regions(sequence, outlier)

        # Try direct repeats first
        direct_matches = self.find_exact_matches(
            left_flank, right_flank, is_inverted=False
        )

        # Try inverted repeats if no direct matches
        inverted_matches = []
        if not direct_matches:
            inverted_matches = self.find_exact_matches(
                left_flank, right_flank, is_inverted=True
            )
        ...
        left_repeat = Repeat(
            left_repeat_start, left_repeat_end, best_match[3], repeat_type
        )
        right_repeat = Repeat(
            right_repeat_start, right_repeat_end, best_match[3], repeat_type
        )
...
def process_contig_fast(
    ...
    try:
        composition_analyzer = FastCompositionAnalyzer(
            params["window_size"], params["step_size"]
        )
        outlier_detector = FastOutlierDetector(params["score_threshold"])
        ...
        significant_windows = outlier_detector.filter_significant_outliers(
            scored_windows
        )
        ...
        for outlier in merged_outliers:
            ...
            if repeat_pair:
                ...
                corrected_outlier = Outlier(
                    ...
                    outlier.gc_score,
                    outlier.kmer_score,
                    outlier.combined_score,
                    outlier.confidence,
                )
                predictions.append(
                    Prediction(corrected_outlier, left_repeat, right_repeat)
                )
...
class FastGenomicOutlierPipeline:
    def __init__(
        self, min_contig_length, window_size, step_size, score_threshold, n_threads
    ):
        ...
        try:
            for record in SeqIO.parse(fasta_file, "fasta"):
                if len(record.seq) >= self.min_contig_length:
                    contigs.append(
                        ContigInfo(record.id, str(record.seq), len(record.seq))
                    )

            print(
                f"Loaded {len(contigs)} contigs >= {self.min_contig_length} bp",
                file=sys.stderr,
            )
            return contigs
    ...
    def process_contigs_parallel(self, contigs: List[ContigInfo]) -> List[Prediction]:
        ...
        print(
            f"Processing {total_contigs} contigs using {self.n_threads} threads...",
            file=sys.stderr,
        )
    ...
    def run_pipeline(self, fasta_file: str, output_file: str):
        print(
            "Starting Fast Compositional Outlier Detection Pipeline...", file=sys.stderr
        )
        ...
        with open(output_file, "w") as f:
            ...
            for i, pred in enumerate(predictions, 1):
                ...
                if right_repeat.type == "inverted":
                    right_repeat_sequence = Seq(
                        right_repeat_sequence
                    ).reverse_complement()
                ...
                # Write left repeat
                left_info = (
                    f"{pred_name}:{outlier_coords}|"
                    f"{left_repeat.type}_1|{left_repeat.sequence}|"
                    f"{score_info}|{confidence_info}"
                )
                f.write(
                    f"{outlier.contig}\t{left_repeat.start}\t{left_repeat.end}\t{left_info}\n"
                )
                ...
                f.write(
                    f"{outlier.contig}\t{right_repeat.start}\t{right_repeat.end}\t{right_info}\n"
                )
...
def main():
    ...
    parser.add_argument("--output_bed", help="Output BED file with predictions")
    parser.add_argument(
        "--score-threshold",
        default=2.0,
    ...
    parser.add_argument(
        "--threads", help="Number of threads to use (default: auto-detect, max 4)"
    )
