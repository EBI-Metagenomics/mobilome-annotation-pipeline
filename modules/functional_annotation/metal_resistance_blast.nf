process METAL_RESISTANCE_BLAST {
    tag "$meta.id"
    ...
    output:
    tuple val(meta), path("*.metal.m8"), emit: blast_results
    tuple val(meta), path("*.metal.filtered"), emit: filtered_results
    ...
    filter_blast_results.py \\
        --input ${prefix}.metal.m8 \\
        --output ${prefix}.metal.filtered \\
        --threshold 0.64 \\
        --type protein
    ...
    def prefix = task.ext.prefix ?: "${meta.id}"
    """
    touch ${prefix}.metal.m8
    touch ${prefix}.metal.filtered
